...
pub struct QueryRenderSplit {
    ...
    pub render_ast: ExtractedRender,
}

/// Extracted render expression - either single view or multiple named views
#[derive(Debug, Clone)]
...
impl ExtractedRender {
    /// Get reference to the inner expression for single views.
    pub fn as_single(&self) -> Option<&Expr> {
        match self {
            ExtractedRender::Single(expr) => Some(expr),
            ExtractedRender::Named(_) => None,
        }
    }
}
...
fn extract_render_from_module(module: &mut ModuleDef) -> Result<ExtractedRender> {
        ...
        ExprKind::Pipeline(pipeline) => {
            ...
            if let Some(last) = pipeline.exprs.last() {
                if is_render_call(last) {
                    ...
                    let render_expr = pipeline.exprs.pop().unwrap();

                    // Check if it's a views() call
                    if let ExprKind::FuncCall(func_call) = &render_expr.kind {
                        if let Some(first_arg) = func_call.args.first() {
                            if is_views_call(first_arg) {
                                ...
                                return Some(ExtractedRender::Named(extract_named_views(
                                    first_arg,
                                )));
                            }
                        }
...
/// Check if an expression is a views() function call
...
/// Extract named views from a views() call expression
...
render (list item_template:(block indent:10))
        ...
        let expr = split.render_ast.as_single().expect("Expected single view");
        assert!(is_render_call(expr));

        // Convert to JSON for inspection
        let json = prql_ast_to_json(expr).unwrap();
        match json {
            Value::Object(obj) => {
...
render (tree parent_id:parent_id sortkey:sort_key item_template:this.ui)
    ...
    #[test]
    fn test_single_line_pipeline_requires_pipe() {
        // PRQL requires `|` or newline between pipeline stages.
        // Without it, `render` is not parsed as a separate stage.
        let without_pipe = r#"from children render (list item_template:(text this.id))"#;
        assert!(
            split_prql_at_render(without_pipe).is_err(),
            "Single-line without pipe should fail to find render()"
        );

        let with_pipe = r#"from children | render (list item_template:(text this.id))"#;
        let split = split_prql_at_render(with_pipe).unwrap();
        let expr = split.render_ast.as_single().expect("Expected single view");
        assert!(is_render_call(expr));
    }
}
