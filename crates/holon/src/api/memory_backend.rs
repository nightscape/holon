...
use holon_api::{ApiError, Block, BlockChange, BlockContent, Change, ChangeOrigin, StreamPosition};
...
impl Lifecycle for MemoryBackend {
    async fn create_new(doc_id: String) -> Result<Self, ApiError> {
        let doc_uri = document_uri(&doc_id);
        ...
        let first_block = Block::new_text(first_block_id.clone(), doc_uri.clone(), String::new());
...
impl CoreOperations for MemoryBackend {
    ...
    async fn get_all_blocks(
        ...
        // Start traversal from document roots
        // Find all parent keys that are document URIs and traverse their children
        for parent_key in state.children_by_parent.keys() {
            if is_document_uri(parent_key) {
    ...
    async fn create_block(
        ...
        // Create block using flattened structure
        let block = Block::from_block_content(block_id.clone(), parent_id.clone(), content);

        state.blocks.insert(block_id.clone(), block.clone());
    ...
    async fn move_block(
        ...
        if !state.blocks.contains_key(&new_parent) || state.deleted_ids.contains(&new_parent) {
            return Err(ApiError::BlockNotFound {
                id: new_parent.clone(),
            });
        }
        ...
        if let Some(ref after_id) = after {
            if let Some(pos) = target_children.iter().position(|cid| cid == after_id) {
    ...
    async fn create_blocks(&self, blocks: Vec<NewBlock>) -> Result<Vec<Block>, ApiError> {
        ...
        for new_block in blocks {
            ...
            if !state.blocks.contains_key(&new_block.parent_id)
                || state.deleted_ids.contains(&new_block.parent_id)
            {
                return Err(ApiError::BlockNotFound {
            ...
            // Create block using flattened structure
            let block = Block::from_block_content(
                block_id.clone(),
