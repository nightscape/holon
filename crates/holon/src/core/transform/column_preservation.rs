...
use anyhow::Result;
use prqlc::pr::{Expr, ExprKind, ModuleDef, StmtKind};

use super::ast_builders::{create_ident_path, create_select, has_append_in_expr, is_ident_named};
...
fn create_select_this_star() -> Expr {
    let this_star = create_ident_path(vec!["this".to_string(), "*".to_string()]);
    create_select(vec![this_star])
}
