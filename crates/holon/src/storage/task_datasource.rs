use crate::core::datasource::{CrudOperations, DataSource, FieldDelta, OperationResult, Result};
...
impl CrudOperations<Task> for InMemoryTaskStore {
    async fn set_field(&self, id: &str, field: &str, value: Value) -> Result<OperationResult> {
        ...
        if let Some(pos) = updated_flat.iter().position(|t| t.id == id) {
            ...
            // Clone value before using it (since we need it for FieldDelta)
            let value_clone = value.clone();
            match field {
                "title" => {
                    if let Value::String(s) = &value {
                        updated_flat[pos].title = s.clone();
                    }
                }
                "completed" => {
                ...
                "parent_id" => match &value {
                    Value::String(s) => updated_flat[pos].parent_id = Some(s.clone()),
            ...
            *tasks = Self::rebuild_hierarchy(updated_flat);
            ...
            // Create FieldDelta describing the change
            let changes = vec![FieldDelta::new(id, field, old_value.clone(), value_clone)];
    ...
    async fn create(&self, fields: HashMap<String, Value>) -> Result<(String, OperationResult)> {
        ...
        *tasks = Self::rebuild_hierarchy(flat);
        ...
        // Create FieldDeltas describing the new task
        let mut changes = vec![
            FieldDelta::new(&id, "id", Value::Null, Value::String(id.clone())),
            FieldDelta::new(&id, "title", Value::Null, Value::String(title)),
            FieldDelta::new(&id, "completed", Value::Null, Value::Boolean(completed)),
        ];
        if let Some(ref pid) = parent_id {
            ...
            changes.push(FieldDelta::new(&id, "parent_id", Value::Null, Value::Null));
        }
        ...
        Ok((
            ...
            OperationResult::new(
                changes,
                __operations_crud_operations::delete_op(
                    "", // Will be set by OperationProvider
                    &id,
                ),
            ),
    ...
    async fn delete(&self, id: &str) -> Result<OperationResult> {
        ...
        if let Some(pos) = flat.iter().position(|t| t.id == id) {
            ...
            *tasks = Self::rebuild_hierarchy(flat);
            ...
            // Create FieldDelta for deletion
            ...
            Ok(OperationResult::new(
                changes,
                __operations_crud_operations::create_op(
                    "", // Will be set by OperationProvider
                    create_fields,
                ),
            ))
