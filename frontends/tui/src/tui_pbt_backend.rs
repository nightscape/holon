...
use holon_api::Value;
use holon_api::{is_document_uri, ApiError, Block, HasSchema, NO_PARENT_ID};
...
impl TuiR3blPbtBackend {
    ...
    async fn get_children_for_block(&self, parent_id: &str) -> Result<Vec<String>, ApiError> {
        ...
        // Map NO_PARENT_ID to NULL for database queries
        let sql = if parent_id == NO_PARENT_ID {
    ...
    pub async fn ensure_schema(&self) -> Result<(), ApiError> {
        ...
        let result = engine.execute_query(check_sql, HashMap::new(), None).await;
        ...
        if result.is_err() || result.unwrap().is_empty() {
            ...
            engine
                .execute_query(create_table_sql, HashMap::new(), None)
                .await
                .map_err(|e| ApiError::InternalError {
            ...
            let root_exists = {
                let check_sql = format!("SELECT id FROM blocks WHERE id = '{}'", NO_PARENT_ID);
                let result = engine.execute_query(check_sql, HashMap::new(), None).await;
            ...
            if !root_exists {
                ...
                root_entity.insert("id".to_string(), Value::String(NO_PARENT_ID.to_string()));
                root_entity.insert(
                    "parent_id".to_string(),
                    ...
                    .with_backend_write(|backend: &mut _| -> anyhow::Result<()> {
                        ...
                        tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current()
                                .block_on(backend.insert(&Block::schema(), root_entity_clone))
                        })
                ...
                use holon::storage::gen_key_between;
                    ...
                    .with_backend_write(|backend: &mut _| -> anyhow::Result<()> {
                        tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current().block_on(
                                backend.insert(&Block::schema(), first_child_entity_clone),
                            )
                        })
                        .map_err(|e| anyhow::anyhow!("{}", e))
                    })
...
impl CoreOperations for TuiR3blPbtBackend {
    ...
    async fn create_block(
        ...
        // Map NO_PARENT_ID to NULL for database
        if parent_id == NO_PARENT_ID {
        ...
        engine
            .with_backend_write(|backend: &mut _| -> anyhow::Result<()> {
                tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(backend.insert(&Block::schema(), entity_clone))
                })
                .map_err(|e| anyhow::anyhow!("{}", e))
            })
            .await
    ...
    async fn move_block(
        ...
        if new_parent != NO_PARENT_ID {
            ...
            let check_results = engine
                .execute_query(check_sql, HashMap::new(), None)
                .await
                .map_err(|e| ApiError::InternalError {
