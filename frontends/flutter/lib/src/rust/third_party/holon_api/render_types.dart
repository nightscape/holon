...
// These function are ignored because they are on traits that is not defined in current crate (put an empty `#[frb]` on it to unignore): `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `clone`, `eq`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`, `fmt`
// These functions are ignored (category: IgnoreBecauseExplicitAttribute): `get_root_mut`, `new`, `root`
...
@freezed
sealed class FilterExpr with _$FilterExpr {
  const FilterExpr._();

  /// Column equals literal: this.region == "sidebar"
  const factory FilterExpr.eq({required String column, required Value value}) =
      FilterExpr_Eq;

  /// Column not equals
  const factory FilterExpr.ne({required String column, required Value value}) =
      FilterExpr_Ne;

  /// Boolean AND of filters
  const factory FilterExpr.and(List<FilterExpr> field0) = FilterExpr_And;

  /// Boolean OR of filters
  const factory FilterExpr.or(List<FilterExpr> field0) = FilterExpr_Or;

  /// Always true (no filter)
  const factory FilterExpr.all() = FilterExpr_All;
}
...
class RenderSpec {
  ...
  final Map<String, ViewSpec> views;
...
class ViewSpec {
  ...
  final RenderExpr structure;

  const ViewSpec({this.filter, required this.structure});

  @override
  int get hashCode => filter.hashCode ^ structure.hashCode;
