...
class TreeViewWidgetContent extends ConsumerWidget {
    ...
    return treeState.treeController == null
        ...
        : AnimatedTreeView<Map<String, dynamic>>(
            ...
            nodeBuilder: (BuildContext buildContext, TreeEntry<Map<String, dynamic>> entry) {
              ...
              if (queryParams != null) {
                // Per-node state: each node watches only its own row data
                // This enables fine-grained rebuilds when row data changes
                // NOTE: We intentionally don't pass rowCache as a prop -
                // TreeNodeWidget fetches it via ref.read() to avoid rebuilds
                ...
                nodeWidget = buildTemplate(template.expr, nodeContext);
              }
  ...
  /// For single-table queries (no ui column), returns the first template.
  RowTemplate _getTemplateForRow(Map<String, dynamic> row) {
    final uiIndex = row['ui'] as int?;
    if (uiIndex == null) {
      // Single-table query: no ui column, use first (and only) template
      return rowTemplates.first;
    }
