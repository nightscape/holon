...
class RenderContext {
  ...
  /// When set, tree nodes can use select() to watch only their specific row.
  final ReactiveQueryParams? queryParams;

  /// For root-level columns() that render the full screen layout.
  /// When true, columns() renders as Stack with sidebar positioning.
  final bool isScreenLayout;

  /// Drawer open/close state for animated sidebar.
  /// Only used when isScreenLayout is true.
