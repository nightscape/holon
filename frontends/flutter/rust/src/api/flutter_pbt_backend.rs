...
// Note: Block comes from holon_api directly, not re-exported here
pub use holon::api::types::Traversal;
pub use holon_api::block::Block;
...
impl CoreOperations for FlutterPbtBackend {
    ...
    async fn create_block(
        ...
        Ok(Block::from_block_content(block_id, parent_id, content))
    }
    ...
    async fn create_blocks(&self, blocks: Vec<NewBlock>) -> Result<Vec<Block>, ApiError> {
        ...
        for new_block in blocks {
            ...
            created.push(Block::from_block_content(
                block_id,
                new_block.parent_id,
                new_block.content,
            ));
